...
class AcademicYearRepository:
    ...
    async def activate_academic_year(self, academic_year_id: UUID) -> CRUDResult[AcademicYear]:
        """Active une année académique spécifique (la passe à active=True).
        
        IMPORTANT : Appeler deactivate_academic_year() AVANT cette méthode pour respecter
        la contrainte d'unicité (une seule année active à la fois).
        
        Args:
            academic_year_id: L'identifiant de l'année académique à activer.
        
        Returns:
            CRUDResult: L'année académique activée.
        """
        try:
            ...
            academic_year.active = True
            await self.db.commit()
            await self.db.refresh(academic_year)
 
            logger.info(f"Academic year with id {academic_year_id} activated successfully")
            return CRUDResult.crud_success(academic_year, 200)
        except IntegrityError as e:
            # Violaion de la contrainte uq_academic_year_active — une autre année est déjà active
            return await RepositoriesUtils.traiter_integrity_error(e, self.db, logger, AcademicYear)
        except Exception as e:
            await self.db.rollback()
            return await RepositoriesUtils.traiter_exception_inconnue(e, self.db, logger)
