...
class PostRepository:
    ...
    async def save_post_media(
        self, post_media: PostMedia, in_transaction: bool
    ) -> CRUDResult[PostMedia]:
        """
        Enregistre un media lié à un post dans la bd
        Args:
            post_media: L'objet PostMedia contenant les informations du media à enregistrer
        ...
        """
        try:
            self.bd_session.add(post_media)
            await self.bd_session.flush()
            await self.bd_session.refresh(post_media)

            if in_transaction:
                logger.warning("Media de post insert mais pas commit en Base, MODE TRANSACTION")
            else:
                await self.bd_session.commit()
