...
from app.cache.academic_year_cache import AcademicYearCache
...
from app.schemas.academic_year_schemas import AcademicYearCreateRequest, AcademicYearListResponse, AcademicYearResponse, AcademicYearUpdateRequest
...
class AcademicYearService:
    ...
    def __init__(self, db : AsyncSession, redis:CacheWrapper, ):
        ...
        self.academic_year_cache = AcademicYearCache(redis)
    ...
    async def get_active_academic_year(self) -> ServiceResult[AcademicYearResponse]:
        """Récupère l'année académique active — cache dédié en priorité.
        
        Returns:
            ServiceResult[AcademicYearResponse]: Les données de l'année académique active.
        """
        try:
            cached_active = await self.academic_year_cache.get_active_academic_year()
            if cached_active is not None:
                logger.info(f"Année académique active trouvée dans le cache")
                return ServiceResult.service_success(cached_active, status._200_STATUS_SUCCESS, msg.ACADEMIC_YEAR_SERVICE)
        except Exception as e:
            logger.warning(f"Erreur cache lors de la récupération de l'année académique active: {e}")
 
        academic_year_result = await self.academic_year_repository.get_active_academic_year()
        if academic_year_result.is_error():
            return ServiceResult.service_error(academic_year_result.error, academic_year_result.status_code, msg.ACADEMIC_YEAR_SERVICE)
 
        academic_year_response = AcademicYearResponse.model_validate(academic_year_result.data)
 
        try:
            await self.academic_year_cache.set_active_academic_year_in_cache(academic_year_response, ACADEMIC_YEAR_CACHE_TTL)
            await self.academic_year_cache.set_academic_year_in_cache(academic_year_response, ACADEMIC_YEAR_CACHE_TTL)
            logger.info(f"Année académique active sauvegardée dans le cache pour l'ID {academic_year_response.id}")
        except Exception:
            logger.warning(f"Impossible de sauvegarder l'année académique active dans le cache")
    ...
    async def soft_delete_academic_year(self, academic_year_id: UUID) -> ServiceResult[StringMessage]:
        """Supprime une année académique (soft delete) et invalide tous ses caches.
        
        Args:
            academic_year_id: L'identifiant de l'année académique à supprimer.
            
        Returns:
            ServiceResult[StringMessage]: Un message indiquant le résultat de l'opération.
        """
        existing_academic_year_result = await self.academic_year_repository.get_academic_year_by_id(academic_year_id)
        if existing_academic_year_result.is_error():
            return ServiceResult.service_error(existing_academic_year_result.error, existing_academic_year_result.status_code, msg.ACADEMIC_YEAR_SERVICE)
        ...
        was_active = academic_year_to_delete.active  # On note si c'était l'année active
        ...
        try:
            ...
            # Invalider le cache par ID
            try:
                await self.academic_year_cache.delete_academic_year_from_cache(str(academic_year_id))
                # Si c'était l'année active, invalider aussi le cache actif
                if was_active:
                    await self.academic_year_cache.delete_active_academic_year_from_cache()
            ...
            return ServiceResult.service_success(
                StringMessage(message="Année académique supprimée avec succès."),
                status._200_STATUS_SUCCESS,
                msg.ACADEMIC_YEAR_SERVICE
            )
        except Exception as e:
    ...
    async def set_active_academic_year(self, academic_year_id: UUID) -> ServiceResult[AcademicYearResponse]:
        """Active une année académique spécifique.
        
        Args:
            academic_year_id: L'identifiant de l'année académique à activer.
            
        Returns:
            ServiceResult[AcademicYearResponse]: L'année académique nouvellement active.
        """

        existing = await self.academic_year_repository.get_academic_year_by_id(academic_year_id)
        if existing.is_error():
            return ServiceResult.service_error(existing.error, existing.status_code, msg.ACADEMIC_YEAR_SERVICE)
 

        active_year = await self.academic_year_repository.get_active_academic_year()
        if active_year.is_success():
            # Pas besoin de désactiver si c'est déjà la même année
            if active_year.data.id == academic_year_id:
                academic_year_response = AcademicYearResponse.model_validate(active_year.data)
                return ServiceResult.service_success(academic_year_response, status._200_STATUS_SUCCESS, msg.ACADEMIC_YEAR_SERVICE)
            ...
            # Invalider le cache de l'ancienne année active
            try:
                await self.academic_year_cache.delete_active_academic_year_from_cache()
                await self.academic_year_cache.delete_academic_year_from_cache(str(active_year.data.id))
            except Exception:
                logger.warning("Impossible d'invalider le cache de l'ancienne année active")
 

        activate_result = await self.academic_year_repository.activate_academic_year(academic_year_id)
        if activate_result.is_error():
            return ServiceResult.service_error(activate_result.error, activate_result.status_code, msg.ACADEMIC_YEAR_SERVICE)
 
        academic_year_response = AcademicYearResponse.model_validate(activate_result.data)
 
        # Mettre à jour les deux caches
        try:
            await self.academic_year_cache.set_academic_year_in_cache(academic_year_response, ACADEMIC_YEAR_CACHE_TTL)
            await self.academic_year_cache.set_active_academic_year_in_cache(academic_year_response, ACADEMIC_YEAR_CACHE_TTL)
            logger.info(f"Année académique {academic_year_id} activée et sauvegardée dans le cache")
        except Exception:
            logger.warning(f"Impossible de sauvegarder l'année académique active {academic_year_id} dans le cache")
