...
class PostService:
	...
	async def verify_post_can_been_processed(
		...
		cache: CacheWrapper,
		data_to_vrify: Union[CreatePostFullData, CreateMediaUploadIntentFullData],
		user_obj: ReadUser,
	) -> ServiceResult[str]:
		...
		if data_to_vrify.club_id:
			...
			res = await club_member_svc.service_check_membership(
				data_to_vrify.club_id, user_obj.id,
			)
			if res.is_error():
				return ServiceResult.service_error(
					message=res.error,
		...
		if data_to_vrify.event_id:
			evt = await EventService(bd_session, cache).service_find_event_by_id(
				data_to_vrify.event_id,
			)
			if evt.is_error():
				return ServiceResult.service_error(
					message=evt.error,
	...
	async def service_create_text_post(
		...
		post_full_data = CreatePostFullData.model_validate(
			post_data, from_attributes=True,
		)

		verif = await self.verify_post_can_been_processed(
			self.db, self._cache, post_full_data, user,
		)
	...
	def _format_post_infos(post: Post, user_id: str) -> ReadPost:
		...
		user_info = PostAuthorSchema(**post.author.__dict__)

		if post.club:
			club_info = PostClubSchema(**post.club.__dict__)
		if post.event:
			event_info = PostEventSchema.model_validate(
				post.event, from_attributes=True,
			)

		if post.classe:
			classe_info = PostClasseSchema.model_validate(
				post.classe, from_attributes=True,
			)
				...
				medias_list.append(
					PostMediaSchema(
						...
						hls_master_url=(
							...
							else MediaReadStorage.generate_post_read_hls_url(
						...
						image_medium_url=(
							...
							else MediaReadStorage.generate_post_medium_post_image_url(
								key, create_stream_token(key, user_id, preffix_path),
							)
						),
						image_high_url=(
							None
							if media.media_type == MediaType.VIDEO
							else MediaReadStorage.generate_post_high_quality_post_image_url(
								key, create_stream_token(key, user_id, preffix_path),
							)
						),
					),
	...
	async def service_get_feed(
		...
		current_academic_year_request = await AcademicYearService(
			self.db, self._cache,
	...
	@classmethod
	async def verify_can_interract_with_post(
		cls,
		post_id: UUID,
