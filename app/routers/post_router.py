...
from app.schemas.post_upload_schemas import (
	PostMediaUploadIntentResponse, CreateMediaUploadIntent,
...
async def ws_post_processing_info(
	...
	intent_id: str = Query(
		..., description="L'id d'intent d'upload de média pour lequel on veut suivre le post-traitement",
	),
):
	"""
...
async def get_feed(
	...
	current_user: Annotated[ReadUser, Depends(get_current_user)],
	post_service: Annotated[PostService, Depends(get_post_service)],
	exclude_viewed: Annotated[
		bool,
		Query(
			description="Exclure les posts déjà vus par l'utilisateur (Jsp mais çà peut être utile pour le "
			            "debug ou pour une feature de 'revoir les posts déjà vus' ou un truc du genre, à voir)",
		)] = True,
...
async def like_post(
	...
	result = await like_service.save_post_like(
		post_id=post_id,
		user_id=current_user.id,
		user_classe_id=current_user.classe.id if current_user.classe else None,
...
async def like_post(
	...
	result = await like_service.delete_post_like(
		post_id=post_id,
		user_id=current_user.id,
		user_classe_id=current_user.classe.id if current_user.classe else None,
		user_role=current_user.role,
