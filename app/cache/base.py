...
from app.cache.cache_keys import AvailableCacheKeys
...
class CacheWrapper:
    ...
    async def save_in_cache(self, key: AvailableCacheKeys, value: str, expire_seconds: Optional[int | timedelta] = None) -> None:
        """
        Enregistre une valeur dans le cache avec une clé spécifique et une durée d'expiration optionnelle
        Args:
            key: La clé sous laquelle la valeur doit être enregistrée dans le cache, définie dans AvailableCacheKeys
        ...
        await self._connection.set(key.value, value, ex=expire_seconds)


    async def delete_in_cache(self, key: AvailableCacheKeys) -> None:
        """
        Supprime une valeur du cache en utilisant une clé spécifique
        Args:
            key: La clé de cache à supprimer, définie dans AvailableCacheKeys

        Returns:
            Que dalle, cette méthode ne retourne rien, elle effectue simplement l'opération de suppression du cache
        """

        await self._connection.delete(key.value)

    async def get_from_cache(self, key: AvailableCacheKeys) -> Optional[str]:
        """
        Récupère une valeur du cache en utilisant une clé spécifique
        Args:
            key: La clé de cache à récupérer, définie dans AvailableCacheKeys

        Returns:
            La valeur associée à la clé de cache si elle existe, sinon None
        """

        return await self._connection.get(key.value)

    async def exists_in_cache(self, key: AvailableCacheKeys) -> bool:
        """
        Vérifie si une clé de cache existe dans Redis
        Args:
            key: La clé de cache à vérifier, définie dans AvailableCacheKeys

        Returns:
            True si la clé de cache existe, sinon False
        """

        return await self._connection.exists(key.value) == 1

    async def expire_in_cache(self, key: AvailableCacheKeys, expire_seconds: Optional[int | timedelta] = None) -> None:
        """
        Met à jour la durée d'expiration d'une clé de cache existante
        Args:
            key: La clé de cache pour laquelle mettre à jour l'expiration, définie dans AvailableCacheKeys
        ...
        await self._connection.expire(key.value, expire_seconds)

    async def ttl_in_cache(self, key: AvailableCacheKeys) -> Optional[int]:
        """
        Récupère le temps restant avant l'expiration d'une clé de cache
        Args:
            key: La clé de cache pour laquelle récupérer le TTL, définie dans AvailableCacheKeys
        ...
        ttl = await self._connection.ttl(key.value)
        return ttl if ttl >= 0 else None

    async def incr_in_cache(self, key: AvailableCacheKeys, amount: int = 1) -> int:
        """
        Incrémente une valeur numérique dans le cache de manière atomique
        Args:
            key: La clé de cache à incrémenter, définie dans AvailableCacheKeys
        ...
        return await self._connection.incr(key.value, amount)

    async def decr_in_cache(self, key: AvailableCacheKeys, amount: int = 1) -> int:
        """
        Décrémente une valeur numérique dans le cache de manière atomique
        Args:
            key: La clé de cache à décrémenter, définie dans AvailableCacheKeys
        ...
        return await self._connection.decr(key.value, amount)
...
class CacheManager:
    ...
    def __aenter__(self) -> CacheWrapper:
        """Permet d'utiliser le CacheManager dans un contexte async avec 'async with'."""
        return self.get_redis_connection_from_pool()

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        """Permet de gérer la sortie du contexte async, en s'assurant que les connexions sont correctement fermées."""
        # Ici, on n'a pas besoin de faire quoi que ce soit de spécial pour fermer les connexions,
        # car le pool de connexions gère cela automatiquement. Cependant, si on avait besoin de faire du nettoyage,
        # c'est ici que ça se passerait.
        pass
