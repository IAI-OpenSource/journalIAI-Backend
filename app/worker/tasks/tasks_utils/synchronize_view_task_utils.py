...
async def add_views_for_user(
    user_id: str,
    redis_conn: PostFeedCache,
    bd: PostViewsRepository,
    logger: Logger,
    mock_viewed_at: datetime,
) -> None:
    try:
        ...
        views_dict = [
            {"user_id": user_id, "post_id": p_id, "viewed_at": mock_viewed_at}
            for p_id in viewed_post
        ]
        ...
        if res.is_error():
            logger.error(
                f"Erreur lors de l'insertion des vues pour user : {user_id} : {res.error}"
            )
            return None
        ...
        logger.error(
            f"Erreur {e.__class__.__name__} lors de l'insertion des vu"
            f"s pour le user : {user_id} : {str(e)})"
        )
        return None


async def insert_views_for_all_users(
    users: List[str], redis_conn: PostFeedCache, batch_size: int, logger: Logger
    ...
    try:
        ...
        return ProcessingResult.error_response(
            f"Erreur {e.__class__.__name__} lors des insertion des vu"
