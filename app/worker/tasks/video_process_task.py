...
def verify_file_is_video(file_path : str) -> bool:
    """
...
def download_file_from_bucket(bucket_name: str, file_path_in_bucket : str, file_destination_path: str) -> None:
    """
...
def upload_processed_file_to_bucket(bucket_name: str, file_path_in_bucket: str, file_local_path: str) -> None:
    """
    ...
    minio_client.fput_object(bucket_name, file_path_in_bucket, file_local_path)

def delete_file_from_bucket(bucket_name: str, file_path_in_bucket: str) -> None:
    """Tout est dans le nom de la fonction"""
    minio_client = MinioClientFactory.get_backend_client()

    minio_client.remove_object(bucket_name, file_path_in_bucket)

def process_video_file_with_ffmpeg(input_path: str, output_path: str) -> None:
...
def process_video_task(raw_object: Object, intent_id: str) -> None:
    ...
    try:
        download_file_from_bucket(raw_object.bucket_name, raw_object.object_name, local_raw_path)

        if not verify_file_is_video(local_raw_path):
            return

        process_video_file_with_ffmpeg(input_path=local_raw_path, output_path=local_processed_path)
        ...
        # TODO: Ajouter la logique ajout bd et cache Redis pour marquer l'intent d'upload video comme traité et stocker les infos de la vidéo traitée (url dans le bucket Minio, métadonnées, etc.) pour que le frontend puisse les récupérer et afficher le post vidéo traité à l'utilisateur
    except Exception as e:
        pass
    finally:
        # Nettoyage local du conteneur (très important pour ne pas saturer le disque !)
