...
class Post(Base, IntegrityMapperMixin):
    ...
    # Relationships
    author: Mapped["User"] = relationship("User", foreign_keys=[author_id], back_populates="posts", uselist=False)
    club: Mapped[Optional["Club"]] = relationship("Club", foreign_keys=[club_id], back_populates="posts", uselist=False)
    event: Mapped[Optional["Event"]] = relationship("Event", foreign_keys=[event_id], back_populates="posts", uselist=False)
    comments: Mapped[list["Comment"]] = relationship("Comment", back_populates="post", cascade="all, delete-orphan", uselist=True)
